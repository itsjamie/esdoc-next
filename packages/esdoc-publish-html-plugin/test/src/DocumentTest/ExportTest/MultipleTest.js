...
describe('test multiple export', ()=> {
  it('is documented.', ()=>{
    ...
    assert.includes(doc1, '.header-notice [data-ice="importPath"]', `import {TestExportMultiple} from '@itsjamie/esdoc-test-fixture/src/Export/Multiple.js'`);

    const doc2 = readDoc('variable/index.html');
    findParent(doc2, '[id="static-variable-testExportMultiple"]', '[data-ice="detail"]', (doc)=>{
      assert.includes(doc, '[data-ice="importPath"]', `import {testExportMultiple} from '@itsjamie/esdoc-test-fixture/src/Export/Multiple.js'`);
    });
  });
});
